...
require_once"page/session-start.php";
  ...
  // GENERATE RANDOM TOKEN FOR CSRF
    if(empty($_SESSION["blindtoken"])){
    // $_SESSION["token"]=password_hash(random_bytes(32), PASSWORD_DEFAULT) ; // it is only verify using password_verify(password, hash)
      $_SESSION["blindtoken"]= random_bytes(32) ; // it is only verify using password_verify(password, hash)
  }
      $csrf = hash_hmac('sha256', 'This is string for csrf token a', $_SESSION["blindtoken"]);
  ...
  if (e.value === 'low') {
    ...
    console.log(e.value);
     ...
     $("#myPost").attr("method", "GET");      
		...
		<div class="content">
		  <h2 style="text-align:center">Blind SQL Injection Module</h2>
		  			<div class="sign-up">
        ...
        if( isset( $_GET[ 'search' ]) AND $_GET['security']=='low'){
         echo "<span style='color:red'>Low Security: </span>"; 
          ...
          // $security= $_GET['security'];
           ...
           $Blind= new BlindSecurity();
          $Search = $Blind->low($id); 

}                           // To check Medium security 
elseif( isset( $_POST[ 'search' ]) AND $_POST['security']='medium' ){
          echo "<span style='color:red'>Medium Security: </span>";
          $id = $_POST['id'];
          // $security= $_POST['security'];

          $Blind= new BlindSecurity();
          $Search = $Blind->middle($id); 
}
